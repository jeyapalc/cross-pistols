...
export default function TargetDisplay({ status, fullScreen = false, subtitle = "" }) {
    ...
    return (
        <div className={`relative flex justify-center items-center perspective-1000 ${fullScreen ? 'fixed inset-0 z-[60] bg-transparent pointer-events-none' : 'w-full h-80 mb-8'}`}>
            {/* 3D Container */}
            <div
            ...
            {!isFacing && status === STATUS.BRIEFING && (
                <div className="absolute inset-x-4 sm:inset-x-12 bottom-32 sm:bottom-48 flex flex-col items-center justify-center z-30 pointer-events-none p-4">
                    ...
                    {/* Subtitle Box */}
                    <div className="hud-border max-w-4xl w-full px-6 py-4 sm:px-8 sm:py-6 border-emerald-500/30 bg-[#060608]/90 backdrop-blur-md shadow-[0_0_30px_rgba(16,185,129,0.15)]">
                        <div className="hud-crosshair-v"></div>
                        <p className="text-sm sm:text-xl font-mono text-emerald-50 font-medium leading-relaxed tracking-wide text-center uppercase drop-shadow-lg">
                            {subtitle}
                        </p>
                    </div>
                </div>
